...
//         {
//             auto it = std::find_if(m_coins.begin(), m_coins.end(), [m_register_coin_value](const Coin& coin) { return coin.value == m_register_coin_value; });
//             if (it == m_coins.end()) return 0xffff;
//         }

//         for (auto i = 0; i < m_coins.size(); m_coins++)
//         {
//             auto delta = abs(m_ir_last - m_coin[i].ir_value);
//             if (delta < 30)
//             {
//                 m_coins[i].counter++;
